...
def driver(r0,vmat0,mu0,Ge,Pe,knotsb,β,α,δ,znow,lnow,critin,critmu):
    ...
    # NOTE 200522: The order of matrices are changed to the 'C' (row-major) order,
    # following to the suggestion by @yirwk
    vmat1 = np.zeros((ne,nb))
    gmat0 = np.zeros((ne,nb))
    ...
    for ie in range(ne):
        for ib in range(nb):
            for jb in range(nb):
                ...
                if cnow>0:
                    umat0[ie,ib,jb] = np.log(cnow)
                else:
                    umat0[ie,ib,jb] = -1e+10
...
#         print([iterin,diff])
    ...
    for ie in range(ne):
        ...
        for ib in range(nb):
            ...
            wb[ie,ib] = (knotsb[kb+1]-kp)/(knotsb[kb+1]-knotsb[kb])

            for je in range(ne):

                ia = nb*ie+ib
                ja = nb*je+kb
                AA[ia,ja]   = wb[ie,ib]*Pe[ie,je]
                AA[ia,ja+1] = (1.0-wb[ie,ib])*Pe[ie,je]
    ...
    while diffmu>critmu:
        ...
        dist[0:nb]    = mu0[0,:]
        dist[nb:2*nb] = mu0[1,:]
#         dist = np.reshape(mu0,(2*nb,1),order='F') # NOTE: cannot be used with numba
        dist = AA.T@dist
        mu1[0,:] = dist[0:nb]
        mu1[1,:] = dist[nb:2*nb]
...
vmat0 = np.zeros((ne,nb))
mu0   = np.ones((ne,nb))/(nb*ne)
